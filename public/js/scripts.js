...
// Function to show/hide "Other" role input based on selection
document.addEventListener("DOMContentLoaded", function() {
    const otherRadio = document.getElementById("other");
    const otherRoleContainer = document.getElementById("other-role-container");
    
    // Listen for changes to role radio buttons
    document.querySelectorAll("input[name='Role']").forEach((radio) => {
        radio.addEventListener("change", function() {
            if (otherRadio.checked) {
                otherRoleContainer.style.display = "block"; // Show input if "Other" is selected
            } else {
                otherRoleContainer.style.display = "none"; // Hide input if "Other" is not selected
                document.getElementById("otherRole").value = ""; // Clear "Other" input field
            }
        });
    });
});
